...
const Poi = ({}: PoiProps) => {
  ...
  const setWaypoints = () => {
    ...
    const sourceWaypoint: any = selectedPlaces.filter(
      (place: any) => place.id === source
    );
    const destinationWaypoint = selectedPlaces.filter(
      (place: any) => place.id === destination
    );
  ...
  useEffect(() => {
    {
      categories.forEach((category: any) => {
        fetch(
          ...
          .then((data) => {
            const shouldUpdate = !(
              interests.filter((i) => i.category === category).length > 0
            );
      ...
      {!selected ? (
        <>
          ...
          {interests?.map((interest, key) => (
            <div key={key} className="ml-16">
              <h1 className="text-green-500 uppercase text-5xl mt-12 font-bold py-10">
                {interest?.category}
              </h1>
          ...
          <div className="flex justify-center pb-14">
            <button
              className="btn btn-secondary text-center mt-14"
        ...
        <>
          ...
          <div className="flex w-full gap-16 justify-center items-center">
            <div>
              <h3 className="mb-4 text-center text-2xl text-grey-100">
                Select source
              </h3>
                ...
                <div
                  className={clxs(
                    source === place?.id
                      ...
                      : "bg-primary",
                    "card shadow-xl hover:bg-secondary text-white cursor-pointer w-[350px] mt-4"
                  )}
                  key={key}
